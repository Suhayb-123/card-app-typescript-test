            ...
            {entries.map((entry: Entry, index: number) => {
                return(
                    <div id={entry.id} key={index} className="bg-gray-300 dark:bg-gray-800 shadow-md shadow-gray-500 m-3 p-4 rounded flex flex-col justify-between">
                        ...
                        <p className="text-center text-lg font-light md:mt-2 md:mb-4 mt-1 mb-3 text-gray-900 dark:text-gray-100">{entry.description}</p>
                        ...
                        <div className="text-right text-sm md:text-lg text-gray-900 dark:text-gray-100">
                            <time>Created: {new Date(entry.created_at.toString()).toLocaleDateString()}</time>
                            <br />
                            <time>Scheduled: {new Date(entry.scheduledDate?.toString()).toLocaleDateString()}</time>
                        </div>
                        </section>
                    </div>
                )
